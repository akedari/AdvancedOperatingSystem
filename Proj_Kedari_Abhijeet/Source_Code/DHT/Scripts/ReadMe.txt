...
3. copy the MongoInstall.sh script (/home/abhijeet/workspace/Prog2_Kedari_Abhijeet/DHT) to newly created server instance (via FileZilla OR pscp 
   [pscp -h newHost.txt  -O StrictHostKeyChecking=no -r /home/abhijeet/workspace/Prog2_Kedari_Abhijeet/DHT/MongoInstall.sh /home/ubuntu/])
...
4. copy the code from local machine (/home/abhijeet/workspace/Prog2_Kedari_Abhijeet/DHT) to newly created server instance
...
7. On you local machine create the config file with public IP Addressess of all the instances. This config file will be associated with Evaluation class (/home/abhijeet/workspace/
   Prog2_Kedari_Abhijeet/DHT/src/EvalClass.java)
   AFter creating config file copy it to all servers instances
   pscp -h host.txt -O StrictHostKeyChecking=no -r /home/abhijeet/workspace/Prog2_Kedari_Abhijeet/DHT/config.properties /home/ubuntu/DHT

   (host.txt this file contains ips of server instances we created)

8. type below command on local machine where initServers.sh stored

   pscp -h host.txt -O StrictHostKeyChecking=no -r /home/abhijeet/workspace/Prog2_Kedari_Abhijeet/DHT/initServers.sh /home/ubuntu/
   pssh -h host.txt -O StrictHostKeyChecking=no -o . 'sh ./initServers.sh'
9. type below command on local machine where (/home/abhijeet/workspace/Prog2_Kedari_Abhijeet/DHT/src/EvalClass.java)
